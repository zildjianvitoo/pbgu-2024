...
import { createFinalist } from "@/lib/network/finalist";
...
export default function CreateFinalistForm() {
  const [picture, setPicture] = useState<File>();
  const [pictureUrl, setPictureUrl] = useState<string>();
  const router = useRouter();
  const queryClient = useQueryClient();

  const { mutate: onCreateFinalist } = useMutation({
    mutationFn: (values: CreateFinalistType) => createFinalist(values),
    onSuccess: () => {
      toast.success("Finalis berhasil dibuat!");
